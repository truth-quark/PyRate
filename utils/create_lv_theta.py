...
inc_file = os.path.join(common.SML_TEST_GAMMA, os.path.splitext(common.SML_TEST_DEM_GAMMA)[0] + ".inc")
...
incidence_data = np.ones(shape=(dem_header[ifc.PYRATE_NROWS], dem_header[ifc.PYRATE_NCOLS])) * incidence_angle

elevation_data = np.ones(shape=(dem_header[ifc.PYRATE_NROWS], dem_header[ifc.PYRATE_NCOLS])) * (90.0 - incidence_angle)

shared.write_unw_from_data_or_geotiff(geotif_or_data=incidence_data, dest_unw=inc_file, ifg_proc=1)

shared.write_unw_from_data_or_geotiff(geotif_or_data=elevation_data, dest_unw=elevation_file, ifg_proc=1)
...
shared.write_geotiff(header=header, data_path=elevation_file, dest=dest_lv_theta, nodata=np.nan)

shared.write_geotiff(header=header, data_path=inc_file, dest=dest_inc, nodata=np.nan)
...
np.testing.assert_array_almost_equal(90 - incidence_data, data_elevation, decimal=4)
np.testing.assert_array_almost_equal(incidence_data, data_inc, decimal=4)
